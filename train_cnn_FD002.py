...
from src.features import build_features, assign_conditions, normalize_by_condition
...
train, test, _ = normalize_by_condition(train, test, feature_cols)
...
with mlflow.start_run(run_name=f"cnn_FD002_{EPOCHS}epochs"):
    model = CNNModel(
    ...
    mlflow.log_params({**cnn_cfg, "sequence_length": SEQUENCE_LENGTH})
    ...
    torch.save(model.state_dict(), f'models/cnn_{EPOCHS}epochs.pth')
